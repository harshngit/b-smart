...
import TweetImagePreviewModal from './TweetImagePreviewModal';
import api from '../lib/api';
import tweetCommentService from '../services/tweetCommentService';
import { formatDateFull } from '../lib/utils';

/* ─────────────────────────────────────────────────────────
   Helper – resolve raw media URL to a full URL
───────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────
   CommentRow (unchanged)
───────────────────────────────────────────────────────── */
const CommentRow = ({
  comment, replies, expanded,
  onToggleReplies, onReply, onLike, onDelete,
  onLikeReply, onDeleteReply, currentUserId,
}) => {
        ...
        {expanded && replies[commentId] && replies[commentId].map(reply => (
          <div key={reply._id || reply.id} className="flex gap-3 mt-4 ml-8">
            ...
            <div className="flex-1 text-sm">
              ...
              <div className="flex gap-4 mt-2 text-gray-500 dark:text-gray-400 text-xs">
                <button
                  onClick={() => onLikeReply(commentId, reply._id || reply.id, !reply.is_liked_by_me)}
...
const TweetComponent = ({ tweet }) => {
  ...
  // Resolved image URLs for the lightbox
      ...
      for (const comment of normalizedComments) {
        if (comment.reply_count > 0) {
          ...
          newReplies[comment._id || comment.id] = Array.isArray(repliesData)
            ? repliesData
            : (repliesData?.data || []);
          newExpandedComments[comment._id || comment.id] = false;
        }
      }
      ...
      <div className="bg-white dark:bg-black rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 mb-4">
        ...
        <div className="border-t border-gray-200 dark:border-gray-800">
                ...
                comments.map(comment => (
                  <CommentRow
                    ...
                    onReply={(user) => setReplyTo({
                      ...
                      rootCommentId: comment._id || comment.id,
          ...
          <form
            ...
            className="border-t border-gray-100 dark:border-gray-800 p-3 flex items-center gap-3 md:border-0 md:p-0 md:mt-4"
          >
